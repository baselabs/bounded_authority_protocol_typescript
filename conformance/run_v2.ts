...
//
// Derivation: the v2 profile is the frozen v1 profile plus v:2 payloads, BAP2-* domain separators,
// and the ADR 0028 lte/gte selector kinds; the v2 façade mirrors the v1 façade with exactly those
// deltas, and this runner exercises it against the certified v2 corpus.
...
// drifting silently. The v2 corpus pin (hex
// 6de6289b7f47b0e0a78ea4610e7844a0f1d5247d8eace02ec9cf9841308f13d0) — rotate every pin with
// scripts/regen_corpus_digests.exs in the same change.
const CERTIFIED_INDEX_SHA = "beYom39HsOCnjqRhDnhEoPHVJH2OrOAuyc-YQTCPE9A";
...
function loadCorpus(): { index: Record<string, unknown>; cases: CorpusCase[]; raws: Raws } {
  ...
  for (const entry of index.files as Array<{
    ...
    if (entry.path === "revision.json") {
      ...
      if (sidecar.format !== "bounded-authority-protocol-v2-conformance-corpus-revision") {
...
function dispatchUntrustedKeyLocator(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.untrustedKeyLocator(compact));
...
function dispatchGrantSigningInput(input: Record<string, unknown>): Verdict {
  const r = runThunk(() => v2.grantSigningInput(buildGrantProducer(input)));
  if (r === INVALID || !r.ok) return INVALID;
  return {
...
function buildGrantProducer(input: Record<string, unknown>): v2.GrantProducer {
  return {
...
function toOperationInput(op: Record<string, unknown>): v2.OperationInput {
  const toSelector = (s: unknown): v2.SelectorInput => {
    if (s === "all") return "all";
...
function dispatchProofSigningInput(input: Record<string, unknown>): Verdict {
  ...
  const producer: v2.ProofProducer = {
  ...
  const r = runThunk(() => v2.proofSigningInput(producer));
  if (r === INVALID || !r.ok) return INVALID;
  return {
    protected_segment: utf8(r.value.protectedSegment),
    payload_segment: utf8(r.value.payloadSegment),
...
function dispatchBoundaryAnchorSigningInput(input: Record<string, unknown>): Verdict {
  const r = runThunk(() => v2.boundaryAnchorSigningInput({
...
function dispatchKeyTransitionSigningInput(input: Record<string, unknown>): Verdict {
  const r = runThunk(() => v2.keyTransitionSigningInput({
...
function dispatchAssembleCompact(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.assembleCompact(
    {
      kind: kind as v2.SigningInput["kind"],
...
function dispatchDecodeGrant(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.decodeGrant(compact));
  if (r === INVALID || !r.ok) return INVALID;
...
function dispatchCheckChain(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.checkChain({ rows, ...expected }, expected));
  if (r === INVALID || !r.ok) return INVALID;
...
function buildHistoricalKey(k: Record<string, unknown>, ctx: string): v2.HistoricalPublicKey {
  return {
...
function buildExpectedAnchor(e: Record<string, unknown>, ctx: string): v2.ExpectedAnchor {
  return {
...
function buildExpectedTransition(e: Record<string, unknown>, ctx: string): v2.ExpectedKeyTransition {
  return {
...
function dispatchVerifyHistoricalAnchor(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.verifyHistoricalAnchor(strUtf8(fetchBinary(input, "compact", "verify_historical_anchor")), key, expected));
  if (r === INVALID || !r.ok) return INVALID;
...
function dispatchVerifyKeyTransition(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.verifyKeyTransition(strUtf8(fetchBinary(input, "compact", "verify_key_transition")), currentKey, nextKey, expected));
  if (r === INVALID || !r.ok) return INVALID;
...
function dispatchCheckEnvelope(input: Record<string, unknown>): Verdict {
  ...
  let nonce: v2.ExpectedRequest["nonce"];
  ...
  const r = runThunk(() => v2.checkEnvelope(
...
function dispatchEncodeAnchoredExport(input: Record<string, unknown>): Verdict {
  ...
  const expectedChain: v2.ExpectedChain = {
  ...
  const r = runThunk(() => v2.encodeAnchoredExport({
...
// The v2 corpus splits the export contexts (mirroring the reference): encode cases carry
...
function buildExportExpected(input: Record<string, unknown>): v2.ExpectedExport {
  const expected = (input.expected ?? {}) as Record<string, unknown>;
  const chain = (expected.chain ?? {}) as Record<string, unknown>;
  return {
    chain: {
...
function buildAnchoredExportExpected(input: Record<string, unknown>): v2.ExpectedAnchoredExport {
  const expected = (input.expected ?? {}) as Record<string, unknown>;
  return {
...
function dispatchVerifyAnchoredExport(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v2.verifyAnchoredExport({ chunks, version }, { keys }, expected));
  if (r === INVALID || !r.ok) return INVALID;
  return { chain_id: r.value.chainId };
}
...
// ---- census (the v2 legs: curated == index, two-way; discovery ⊆ declared; verify-import) ----
// v1 census delta (the Python v2 runner is the precedent): the v2 DECLARED census (17 entries) is
// an input to the index from the curated generator (conformance/generators/curated-inputs-v2.json
// `public_key_fingerprints` — facts the generator directory ships that are NOT derivable from case
// bytes), and one curated key emits no case: the case-byte discovery is a strict subset (16 of 17).
// The two-way equality leg is therefore curated == index (both directions), with the case-byte walk
// kept as the fail-closed subset leg (an UNDECLARED key in a case input still aborts), and the
...
function main(): void {
  ...
  console.log(`census: curated-inputs-v2 == index public_key_fingerprints (${(index.public_key_fingerprints as string[]).length} keys, two-way); discovery ⊆ declared; verify-import ⊇ expected-verify-keys`);
  if (fail > 0) {
    console.error(`\n${fail} FAILURE(S) — rerun \`pnpm conformance:v2\` for the case list`);
    process.exit(1);
  }

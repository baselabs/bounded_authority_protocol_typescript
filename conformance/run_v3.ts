#!/usr/bin/env tsx
// Conformance runner for the v3 contract-major profile of @bounded-authority-protocol/verifier. Mirrors
// conformance/run.ts (the v1 runner) and conformance/run_v3.ts: loads the vendored v3 corpus
// snapshot, recomputes EVERY verdict from scratch by calling the v3 façade (src/v3.ts), and
// asserts agreement on all 292 cases. Asserts the index.json SHA-256 at startup (ADR 0014 D4:
// the SDK binds to the exact corpus it was certified against). Runs the key census
// (curated == index two-way; discovery ⊆ declared; verify-import ⊇ expected-verify keys).
//
...
//
// Derivation: the v3 profile is the frozen v1 profile incorporated with the spec/bap-v3.md §2
// substitutions (v:3 payloads, BAP3-* domain separators, alg ES256, the EC P-256 suite: raw
// 65-byte SEC1 keys, the {crv,kty,x,y} proof JWK with RFC 7638 thumbprint, low-S raw r||s
// signatures) plus the v2 §4 selector algebra; the v3 façade mirrors the v2 façade with exactly
// those deltas, and this runner exercises it against the certified v3 corpus.
...
import { _importedFingerprints, _resetCensus } from "../src/es256.js";
import { jwkFromPublicKey, jwkEncodePublic, thumbprintPreimage } from "../src/ec_jwk.js";
import { validateEcPublicKey } from "../src/es256.js";
...
const CORPUS_DIR = join(pathResolve(fileURLToPath(import.meta.url), ".."), "corpus-v3");
...
function loadCorpus(): { index: Record<string, unknown>; cases: CorpusCase[]; raws: Raws } {
  ...
  for (const entry of index.files as Array<{
    ...
    if (entry.path === "revision.json") {
      ...
      if (sidecar.format !== "bounded-authority-protocol-v3-conformance-corpus-revision") {
...
function dispatchJwkThumbprintRaw(input: Record<string, unknown>, raws: Raws): Verdict {
  ...
  if (r === INVALID) return INVALID;
  // The v3 corpus spells the raw digest as its base64url string (the v1/v2 valid cases were
  // verdict-only, so the v2 runner could return the raw Uint8Array).
  return { thumbprint_raw: b64e(r) };
}
function dispatchJwkPublicKeyThumbprintRaw(input: Record<string, unknown>): Verdict {
  ...
  if (r === INVALID) return INVALID;
  // Same base64url spelling as jwk.thumbprint_raw above.
  return { thumbprint_raw: b64e(r) };
}
...
function dispatchUntrustedKeyLocator(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v3.untrustedKeyLocator(compact));
...
function dispatchGrantSigningInput(input: Record<string, unknown>): Verdict {
  const r = runThunk(() => v3.grantSigningInput(buildGrantProducer(input)));
  if (r === INVALID || !r.ok) return INVALID;
  return {
...
function buildGrantProducer(input: Record<string, unknown>): v3.GrantProducer {
  return {
    ...
    operations: ((input.operations as unknown[]) ?? fail("grant_signing_input: operations")).map((op) => toOperationInput(op as Record<string, unknown>)),
  };
}

// A corpus producer selector is plain JSON ({kind, path, value|values} with literal values); the
// façade's SelectorInput carries the tagged algebra. Convert via jsToTagged (the same projection
// the cast_arguments use), preserving the int/float distinction the value tags must carry.
function toOperationInput(op: Record<string, unknown>): v3.OperationInput {
  const toSelector = (s: unknown): v3.SelectorInput => {
    if (s === "all") return "all";
    const o = s as Record<string, unknown>;
    const kind = o.kind;
    if (kind === "all") return { kind: "all" };
    if (kind === "equals") return { kind: "equals", path: o.path as string[], value: jsToTagged(o.value) };
    if (kind === "one_of") return { kind: "one_of", path: o.path as string[], values: (o.values as unknown[]).map(jsToTagged) };
    if (kind === "lte" || kind === "gte") return { kind, path: o.path as string[], value: jsToTagged(o.value) };
    return fail("grant_signing_input: selector kind");
  };
  return { name: op.name as string, selectors: ((op.selectors as unknown[]) ?? fail("grant_signing_input: selectors")).map(toSelector) };
}
...
function dispatchProofSigningInput(input: Record<string, unknown>): Verdict {
  ...
  const producer: v3.ProofProducer = {
  ...
  const r = runThunk(() => v3.proofSigningInput(producer));
  if (r === INVALID || !r.ok) return INVALID;
  return {
    protected_segment: utf8(r.value.protectedSegment),
    payload_segment: utf8(r.value.payloadSegment),
...
function dispatchBoundaryAnchorSigningInput(input: Record<string, unknown>): Verdict {
  const r = runThunk(() => v3.boundaryAnchorSigningInput({
...
function dispatchKeyTransitionSigningInput(input: Record<string, unknown>): Verdict {
  const r = runThunk(() => v3.keyTransitionSigningInput({
...
function dispatchAssembleCompact(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v3.assembleCompact(
    {
      kind: kind as v3.SigningInput["kind"],
...
function dispatchDecodeGrant(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v3.decodeGrant(compact));
  if (r === INVALID || !r.ok) return INVALID;
...
function dispatchEncodeConsumptionEntry(input: Record<string, unknown>): Verdict {
  ...
  if (r === INVALID || !r.ok) return INVALID;
  // The v3 corpus spells the canonical row bytes as base64url (the v2 corpus spelled the raw text).
  return { bytes: b64e(r.value.bytes), hash: b64e(r.value.hash) };
}
...
function dispatchCheckChain(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v3.checkChain({ rows, ...expected }, expected));
  if (r === INVALID || !r.ok) return INVALID;
  return { chain_id: r.value.chainId };
}
...
function buildHistoricalKey(k: Record<string, unknown>, ctx: string): v3.HistoricalPublicKey {
  return {
...
function buildExpectedAnchor(e: Record<string, unknown>, ctx: string): v3.ExpectedAnchor {
  return {
...
function buildExpectedTransition(e: Record<string, unknown>, ctx: string): v3.ExpectedKeyTransition {
  return {
...
function dispatchVerifyHistoricalAnchor(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v3.verifyHistoricalAnchor(strUtf8(fetchBinary(input, "compact", "verify_historical_anchor")), key, expected));
  if (r === INVALID || !r.ok) return INVALID;
  return { anchor_id: r.value.anchorId };
}
...
function dispatchVerifyKeyTransition(input: Record<string, unknown>): Verdict {
  ...
  const r = runThunk(() => v3.verifyKeyTransition(strUtf8(fetchBinary(input, "compact", "verify_key_transition")), currentKey, nextKey, expected));
  if (r === INVALID || !r.ok) return INVALID;
  return { transition_id: r.value.transitionId };
}
...
function dispatchCheckEnvelope(input: Record<string, unknown>): Verdict {
  ...
  let nonce: v3.ExpectedRequest["nonce"];
  ...
  const r = runThunk(() => v3.checkEnvelope(
...
function dispatchEncodeAnchoredExport(input: Record<string, unknown>): Verdict {
  ...
  const expectedChain: v3.ExpectedChain = {
  ...
  const r = runThunk(() => v3.encodeAnchoredExport({
...
// The corpus splits the export contexts (mirroring the reference): encode cases carry
// expected {chain, start_anchor, end_anchor, transitions, bounds} — the digest is an encode
// OUTPUT; verify cases add the mandatory digest + object_version.
function buildExportExpected(input: Record<string, unknown>): v3.ExpectedExport {
  const expected = (input.expected ?? {}) as Record<string, unknown>;
  const chain = (expected.chain ?? {}) as Record<string, unknown>;
  return {
    chain: {
...
function buildAnchoredExportExpected(input: Record<string, unknown>): v3.ExpectedAnchoredExport {
  const expected = (input.expected ?? {}) as Record<string, unknown>;
  return {
    ...buildExportExpected(input),
...
function dispatchVerifyAnchoredExport(input: Record<string, unknown>): Verdict {
  ...
  const expected = buildAnchoredExportExpected(input);
  const r = runThunk(() => v3.verifyAnchoredExport({ chunks, version }, { keys }, expected));
  if (r === INVALID || !r.ok) return INVALID;
  return { chain_id: r.value.chainId };
}
...
function exactPublicJwk(value: Tagged): ReturnType<typeof jwkFromPublicKey> {
  ...
  const crv = value.v.get("crv"); const kty = value.v.get("kty"); const x = value.v.get("x"); const y = value.v.get("y");
  if (!crv || crv.t !== "string" || utf8(crv.v) !== "P-256") fail("jwk: crv");
  if (!kty || kty.t !== "string" || utf8(kty.v) !== "EC") fail("jwk: kty");
  if (!x || x.t !== "string") fail("jwk: x");
  if (!y || y.t !== "string") fail("jwk: y");
  if (value.v.size !== 4) fail("jwk: closed members");
  const xRaw = b64d(utf8(x.v));
  const yRaw = b64d(utf8(y.v));
  if (xRaw.length !== 32) fail("jwk: x width");
  if (yRaw.length !== 32) fail("jwk: y width");
  // Coordinate < p + on-curve (REQ3-KEY-point-on-curve) — same pure-arithmetic gates as the suite.
  const raw = new Uint8Array(65);
  raw[0] = 0x04;
  raw.set(xRaw, 1);
  raw.set(yRaw, 33);
  validateEcPublicKey(raw);
  return { crv: "P-256", kty: "EC", x: utf8(x.v), y: utf8(y.v) };
}
function decodePublicJwkRaw(value: Tagged): Uint8Array {
  const jwk = exactPublicJwk(value);
  return new Uint8Array([0x04, ...b64d(jwk.x), ...b64d(jwk.y)]);
}
...
// boundary). Fingerprints are RFC 7638 thumbprints of the EC JWK member set (spec/bap-v3.md §3.1).

const CURATED_CENSUS_PATH = join(pathResolve(fileURLToPath(import.meta.url), ".."), "curated-inputs-v3.json");
...
function decodeCensusRawKey(value: unknown): Uint8Array | null {
  // The suite's raw public key is the 65-byte uncompressed SEC1 point: base64url of 65 bytes is
  // exactly 87 unpadded characters with the 0x04 prefix byte leading.
...
function runCensus(index: Record<string, unknown>, cases: CorpusCase[]): void {
  ...
  // The two-way leg: every index fingerprint is curated AND every curated fingerprint is indexed.
  const curated = JSON.parse(readFileSync(CURATED_CENSUS_PATH, "utf8")).public_key_fingerprints as string[];
  const curatedSorted = [...new Set(curated)].sort();
  if (JSON.stringify(curated) !== JSON.stringify(curatedSorted)) {
    abort("census: curated public_key_fingerprints not sorted/unique");
  }
  if (JSON.stringify([...new Set(curated)].sort()) !== JSON.stringify(declaredSorted)) {
    const onlyIndex = declaredSorted.filter((fp) => !curated.includes(fp));
    const onlyCurated = curatedSorted.filter((fp) => !declared.includes(fp));
    abort(`census: curated != index public_key_fingerprints\n  only-index=[${onlyIndex.join(",")}]\n  only-curated=[${onlyCurated.join(",")}]`);
  }
  // The case-byte subset leg: every key embedded in a case input must be declared.
  ...
  const undeclared = [...discovery].filter((fp) => !declared.includes(fp)).sort();
  if (undeclared.length > 0) {
    abort(`census: case inputs carry undeclared keys\n  undeclared=[${undeclared.join(",")}]`);
  }
  // Verify-import set: keys actually fed to ES256 verify (tracked in es256.ts). This is a
  ...
  for (const fp of expectedVerify) {
    if (!verifyImport.includes(fp)) {
      abort(`census: key ${fp} declared by a valid verification case but never imported at the ES256 verify boundary`);
    }
  }
}
...
function main(): void {
  ...
  console.log(`census: curated-inputs-v3 == index public_key_fingerprints (${(index.public_key_fingerprints as string[]).length} keys, two-way); discovery ⊆ declared; verify-import ⊇ expected-verify-keys`);
  if (fail > 0) {
    console.error(`\n${fail} FAILURE(S) — rerun \`pnpm conformance:v3\` for the case list`);
    process.exit(1);
  }

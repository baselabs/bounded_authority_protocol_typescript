...
import { jwkFromPublicKey, thumbprintRaw, jwkEncodePublic, jwkDecodePublic, thumbprint } from "./ec_jwk.js";
import { importEcPublicKey, es256Verify, validateEcPublicKey, _resetCensus } from "./es256.js";
import { sha256, sha256Concat } from "./ed25519.js";
...
import type { GrantFacts, EnvelopeFacts, GrantDecoded, ProofDecoded, KeyLocator } from "./facts.js";

// The v3 verification façade (the v3 contract-major profile, suite BAP3-ES256-SHA256 —
// spec/bap-v3.md, ADR 0035). The v3 profile is v1 incorporated by reference with the §2
// substitutions plus the v2 §4 selector algebra, so this façade mirrors the v2 façade with
// exactly these deltas: payloads carry v:3 (every v1/v2 artifact is rejected with the single
// closed error, and vice versa — REQ3-CORE-cross-major-reject), the domain separators are
// BAP3-REQUEST\0 / BAP3-CHAIN\0 / BAP3-ARCHIVE\0EXPORT\0, the protected `alg` is "ES256", and
// the signature suite is ECDSA over NIST P-256 with SHA-256: raw public keys are 65-byte
// uncompressed SEC1 points 0x04||x||y with pure-arithmetic coordinate-range and on-curve
// validation (REQ3-KEY-uncompressed-sec1, REQ3-KEY-point-on-curve), the proof JWK is exactly
// {crv:"P-256", kty:"EC", x, y} with the RFC 7638 thumbprint over that member set
// (REQ3-HEADER-proof-jwk, REQ3-HEADER-thumbprint), and signatures are the RFC 7518 §3.4 raw
// r||s form (exactly 64 bytes) with 0 < r < n, 0 < s ≤ (n−1)/2 — low-S required; Node's
// backend accepts the malleable high-S counterpart, so the profile gate is load-bearing
// (REQ3-SIGNING-raw-rs, REQ3-SIGNING-range, REQ3-SIGNING-low-s). The selector algebra is the
// v2 algebra unchanged: all/equals/one_of plus the inclusive same-tag range kinds lte/gte.
// Each function returns Result<T> = Ok|Err (the {:ok,value}|{:error,:invalid} mirror). No
// authorized/decision surface (rule 1). All claims revalidated at every public entry
// (REQ1-VERIFY-revalidate, incorporated). The v3 certified corpus is the byte-level arbiter;
// the local-loopback HTTP application-proof profile is bound to contract-major 1 and pairs
// with no v3 grant, so this façade exposes no loopback functions (spec/bap-v3.md §2).
...
// BAP3-REQUEST\0 prefix for the request digest (the v3 domain separator; the successor-major
// activation changes every BAP1- separator to its BAP3- form — spec/bap-v3.md §2).
...
// BAP3-CHAIN\0 prefix for consumption-row hashing (the v3 form of ADR 0004 § Consumption rows).
export const ROW_PREFIX = new Uint8Array([
  0x42, 0x41, 0x50, 0x33, 0x2d, 0x43, 0x48, 0x41, 0x49, 0x4e, 0x00, // "BAP3-CHAIN\0"
]);

// BAP3-ARCHIVE\0EXPORT\0 prefix (the v3 form of ADR 0004 § Anchored export; NOT framed).
export const ARCHIVE_PREFIX = strUtf8("BAP3-ARCHIVE\0EXPORT\0");

// The v3 request digest: the shared digest core under the BAP3-REQUEST\0 separator.
function computeRequestDigest(operation: string, castArguments: Tagged, b: Bounds): Uint8Array {
...
export interface TrustedIssuer { readonly keyId: string; readonly publicKey: Uint8Array; } // raw 65 (uncompressed SEC1)
...
export interface HistoricalPublicKey {
  readonly keyId: string; readonly publicKey: Uint8Array; // raw 65 (uncompressed SEC1)
...
// A selector input to the v3 grant producer: either the bare "all" string or a tagged object.
// lte/gte carry a numeric bound in `value` (integer- or float-tagged only; ADR 0028 §2).
export type SelectorInput = "all" | { readonly kind: "all" }
  ...
  | { readonly kind: "lte"; readonly path: string[]; readonly value: Tagged }
  | { readonly kind: "gte"; readonly path: string[]; readonly value: Tagged };
...
export interface ProofProducer {
  readonly holderPublicKey: Uint8Array; // raw 65 (uncompressed SEC1)
...
export interface KeyTransitionProducer {
  ...
  readonly nextKeyId: string; readonly nextPublicKey: Uint8Array; // raw 65 (uncompressed SEC1)
}
...
// The encode-side export context (the reference's ExpectedExport): the caller's chain,
// anchors, transitions, and bounds — the archive digest is an OUTPUT of encode, never an
// expectation, and the stored-object version is out-of-band encode context. The v1 façade
// merged this with the verify-side context; the v2+ reference and corpora keep them split.
...
// The verify-side export context (the reference's ExpectedAnchoredExport): the encode context
// plus the mandatory archive digest and stored-object version.
export interface ExpectedAnchoredExport extends ExpectedExport {
...
// --- the v3 facts (identical field sets to the v1/v2 facts; the version literal is 3) ---
// GrantFacts and EnvelopeFacts carry `version: number` and are shared with the v1 façade;
// these literal-3 variants keep the diagnostic/export fact surfaces major-typed.
...
export interface KeyTransitionFacts {
  ...
  readonly currentKeyFingerprint: Uint8Array; // raw 32
  readonly nextKeyFingerprint: Uint8Array; // raw 32
...
// --- the v3 selector algebra (v2 §4 incorporated: per-request range kinds, contract-major) ---
//
// This façade's selector module. The v1 kind set {all, equals, one_of} extends to exactly
// {all, equals, one_of, lte, gte} on the SAME three recognized member sets (ADR 0021): the
// range kinds use {kind, path, value}. Operand domain is SAME-TAG numeric only — the
// traversed value and the bound must both be integer-tagged or both float-tagged; a
// cross-tag pair never matches, fail closed (ADR 0028 §2 extends
// REQ1-SELECTOR-no-tag-collapse from identity to ordering: an integer 50 never satisfies a
// float 50.0 bound and vice versa). Comparison is INCLUSIVE by numeric value on the finite
// binary64 domain (non-finite floats cannot occur — the bounded decoder rejects them). A
// non-numeric operand (string/boolean/null/array/object at the path, or a non-numeric bound)
// never matches, and a non-numeric bound is rejected at decode. A missing path fails closed
// exactly as equals/one_of. The v1 module (selector.ts) stays closed at its three kinds
// (REQ1-SELECTOR-closed-set); admitting lte/gte in-place would flip an lte-bearing grant from
// Err to Ok (REQ1-EVO-no-verdict-flip).
...
export function parseSelector(obj: Tagged, bounds: Bounds = MAXIMUM_BOUNDS): Selector {
  ...
  if (obj.v.size !== 3) fail("selector: range members");
  const path = parseSelectorPath(obj.v.get("path"), bounds);
  const bound = obj.v.get("value") ?? fail("selector: value");
  if (!isNumericTag(bound)) fail("selector: numeric bound");
  validateSelectorValue(bound, bounds);
  return kind === "lte" ? { kind: "lte", path, value: bound } : { kind: "gte", path, value: bound };
}

function parseSelectorPath(pathV: Tagged | undefined, bounds: Bounds): string[] {
...
function rangeMatches(target: Tagged, bound: Tagged, kind: "lte" | "gte"): boolean {
  if (target.t === "int" && bound.t === "int") return kind === "lte" ? target.v <= bound.v : target.v >= bound.v;
  if (target.t === "float" && bound.t === "float") return kind === "lte" ? target.v <= bound.v : target.v >= bound.v;
...
export function selectorMatches(sel: Selector, castArguments: Tagged): boolean {
  ...
  if (sel.kind === "equals") return bytesEqual(semanticIdentity(sel.value), semanticIdentity(target));
  if (sel.kind === "one_of") {
...
function parseProofHeader(seg: CompactSegments, bounds: Bounds): { holderThumbprint: Uint8Array; holderKey: Uint8Array } {
  ...
  // Closed EC members {crv, kty, x, y} in any member order; every additional member (incl.
  // private d) is invalid — REQ3-HEADER-proof-jwk / REQ3-HEADER-no-private-jwk.
  requireObjectExact(jwkV, ["crv", "kty", "x", "y"], "proof jwk");
  requireStringLit(jwkV, "crv", "P-256", "proof jwk crv");
  requireStringLit(jwkV, "kty", "EC", "proof jwk kty");
  const xV = jwkV.v.get("x")!;
  const yV = jwkV.v.get("y")!;
  if (xV.t !== "string") fail("proof jwk: x string");
  if (yV.t !== "string") fail("proof jwk: y string");
  // Canonical unpadded base64url of exactly 32 bytes each (the RFC 7518 §6.2.1.2/.3
  // fixed-width spelling; base64urlDecode enforces canonicity).
  const xRaw = base64urlDecode(xV.v);
  const yRaw = base64urlDecode(yV.v);
  if (xRaw.length !== 32) fail("proof jwk: x width");
  if (yRaw.length !== 32) fail("proof jwk: y width");
  const rawKey = new Uint8Array(65);
  rawKey[0] = 0x04;
  rawKey.set(xRaw, 1);
  rawKey.set(yRaw, 33);
  // Coordinate < p + on-curve as pure arithmetic BEFORE any backend (REQ3-KEY-point-on-curve;
  // REQ3-BOUNDS-ordering: the JWK member-set, width, < p, and on-curve checks precede the backend).
  validateEcPublicKey(rawKey);
...
function validateGrantPayload(p: Tagged, bounds: Bounds): asserts p is Extract<Tagged, { t: "object" }> {
  ...
  const vV = p.v.get("v")!;
  if (vV.t !== "int" || vV.v !== VERSION) fail("grant: v=3");
...
function validateProofPayload(p: Tagged, bounds: Bounds): asserts p is Extract<Tagged, { t: "object" }> {
  ...
  const vV = p.v.get("v")!;
  if (vV.t !== "int" || vV.v !== VERSION) fail("proof: v=3");
...
function validateAnchorPayload(p: Tagged, payloadBytes: Uint8Array, bounds: Bounds): asserts p is Extract<Tagged, { t: "object" }> {
  ...
  const vV = p.v.get("v")!;
  if (vV.t !== "int" || vV.v !== VERSION) fail("anchor: v=3");
...
function validateTransitionPayload(p: Tagged, payloadBytes: Uint8Array, bounds: Bounds): asserts p is Extract<Tagged, { t: "object" }> {
  ...
  const vV = p.v.get("v")!;
  if (vV.t !== "int" || vV.v !== VERSION) fail("transition: v=3");
...
const SHAPE_EXPECTED_EXPORT: Shape = { fields: { chain: SHAPE_EXPECTED_CHAIN, startAnchor: SHAPE_EXPECTED_ANCHOR, endAnchor: SHAPE_EXPECTED_ANCHOR, transitions: { seq: SHAPE_EXPECTED_TRANSITION }, bounds: SHAPE_BOUNDS_OPT } };
// The verify-side shape adds the two mandatory anchored members (digest + object_version).
const SHAPE_EXPECTED_ANCHORED_EXPORT: Shape = { fields: { chain: SHAPE_EXPECTED_CHAIN, digest: "bytes", startAnchor: SHAPE_EXPECTED_ANCHOR, endAnchor: SHAPE_EXPECTED_ANCHOR, transitions: { seq: SHAPE_EXPECTED_TRANSITION }, objectVersion: "str", bounds: SHAPE_BOUNDS_OPT } };
...
export function untrustedKeyLocator(compact: Uint8Array, bounds?: Bounds): Result<KeyLocator> {
  return trying(() => {
    ...
    const d0 = dots[0]!;
    // Cross-vendor (key-locator empty segments): the reference (v1.ex:24, shared by the v2/v3 façades) only requires exactly 3
    ...
    // (reference v1.ex:27 Json.decode, shared by the v2/v3 façades (header_bytes, bounds) — depth/total_nodes limits honor bounds).
...
export function verifyGrant(compact: Uint8Array, trusted: TrustedIssuer, expected: ExpectedGrant): Result<GrantFacts> {
  return trying(() => {
    ...
    if (!(trusted.publicKey instanceof Uint8Array) || trusted.publicKey.length !== 65) fail("verify_grant: issuer key width");
    validateEcPublicKey(trusted.publicKey);
    ...
    const key = importEcPublicKey(trusted.publicKey, utf8Str(base64urlEncode(fp)));
    if (!es256Verify(seg.signingInput, seg.signature, key)) fail("verify_grant: signature");
    return {
...
export function checkEnvelope(grantCompact: Uint8Array, proofCompact: Uint8Array, expected: ExpectedRequest): Result<EnvelopeFacts> {
  return trying(() => {
    ...
    if (!(t.publicKey instanceof Uint8Array) || t.publicKey.length !== 65) fail("check_envelope: issuer key width");
    validateEcPublicKey(t.publicKey);
    ...
    const gkey = importEcPublicKey(t.publicKey, utf8Str(base64urlEncode(gfp)));
    if (!es256Verify(gseg.signingInput, gseg.signature, gkey)) fail("check_envelope: grant signature");
    ...
    const hkey = importEcPublicKey(holderKey, utf8Str(base64urlEncode(holderThumbprint)));
    if (!es256Verify(pseg.signingInput, pseg.signature, hkey)) fail("check_envelope: proof signature");
...
export function checkChain(chain: ChainInput, expected: ExpectedChain): Result<ChainFacts> {
  return trying(() => {
    ...
    return {
      version: 3 as const, chainId: chain.chainId, firstSequence: chain.firstSequence,
...
function selectorToTagged(s: SelectorInput, b: Bounds): Tagged {
  ...
  if (typeof s === "object" && (s.kind === "lte" || s.kind === "gte")) {
    const path = validatePath(s.path, b);
    if (!isNumericTag(s.value)) fail("selector: numeric bound");
    validateSelectorValue(s.value, b);
    const members = new Map<string, Tagged>([
...
export function proofSigningInput(proof: ProofProducer, bounds?: Bounds): Result<SigningInput> {
  return trying(() => {
    ...
    const b = bounds ?? MAXIMUM_BOUNDS;
    assert(proof.holderPublicKey.length === 65, "proof_signing_input: holder key width");
    validateEcPublicKey(proof.holderPublicKey);
...
function jwkToTagged(jwk: { crv: string; kty: string; x: string; y: string }): Tagged {
  const members = new Map<string, Tagged>([
...
export function boundaryAnchorSigningInput(anchor: BoundaryAnchorProducer, bounds?: Bounds): Result<SigningInput> {
  return trying(() => {
    ...
    assert(anchor.publicKey.length === 65, "anchor_signing_input: public_key width");
    validateEcPublicKey(anchor.publicKey);
...
export function keyTransitionSigningInput(t: KeyTransitionProducer, bounds?: Bounds): Result<SigningInput> {
  return trying(() => {
    ...
    const b = bounds ?? MAXIMUM_BOUNDS;
    assert(t.currentPublicKey.length === 65 && t.nextPublicKey.length === 65, "transition_signing_input: key width");
    validateEcPublicKey(t.currentPublicKey);
    validateEcPublicKey(t.nextPublicKey);
...
export function verifyHistoricalAnchor(compact: Uint8Array, key: HistoricalPublicKey, expected: ExpectedAnchor): Result<AnchorFacts> {
  return trying(() => {
    ...
    assert(key.publicKey.length === 65, "verify_historical_anchor: key width");
    validateEcPublicKey(key.publicKey);
    ...
    const pk = importEcPublicKey(key.publicKey, utf8Str(base64urlEncode(derivedFp)));
    if (!es256Verify(seg.signingInput, seg.signature, pk)) fail("verify_historical_anchor: signature");
    return {
      version: 3 as const, anchorId, anchoredAt, chainId, sequence, chainHash,
...
export function verifyKeyTransition(compact: Uint8Array, oldKey: HistoricalPublicKey, newKey: HistoricalPublicKey, expected: ExpectedKeyTransition): Result<KeyTransitionFacts> {
  return trying(() => {
    ...
    assert(oldKey.publicKey.length === 65 && newKey.publicKey.length === 65, "verify_key_transition: key width");
    validateEcPublicKey(oldKey.publicKey);
    validateEcPublicKey(newKey.publicKey);
    ...
    const pk = importEcPublicKey(oldKey.publicKey, utf8Str(base64urlEncode(derivedFrom)));
    if (!es256Verify(seg.signingInput, seg.signature, pk)) fail("verify_key_transition: signature");
    return {
      version: 3 as const, transitionId, chainId, effectiveAt,
...
export function verifyAnchoredExport(archived: ArchivedObject, keyChain: HistoricalKeyChain, expected: ExpectedAnchoredExport): Result<AnchoredExportFacts> {
  return trying(() => {
    ...
    for (const k of keyChain.keys) {
      ...
      if (!(k.publicKey instanceof Uint8Array) || k.publicKey.length !== 65) fail("verify_anchored_export: key width");
      validateEcPublicKey(k.publicKey);
    }
    {
    ...
    return {
      version: 3 as const, objectVersion: archived.version, chainId: expected.chain.chainId,
...
function verifyAnchorCompact(compact: Uint8Array, key: HistoricalPublicKey, expected: ExpectedAnchor, ctx: string, bounds?: Bounds): void {
  assert(key.publicKey.length === 65, `${ctx}: key width`);
  validateEcPublicKey(key.publicKey);
  ...
  const pk = importEcPublicKey(key.publicKey, utf8Str(base64urlEncode(derivedFp)));
  if (!es256Verify(seg.signingInput, seg.signature, pk)) fail(`${ctx}: signature`);
}
...
function verifyTransitionCompact(compact: Uint8Array, currentKey: HistoricalPublicKey, nextKey: HistoricalPublicKey, expected: ExpectedKeyTransition, ctx: string, bounds?: Bounds): void {
  assert(currentKey.publicKey.length === 65 && nextKey.publicKey.length === 65, `${ctx}: key width`);
  validateEcPublicKey(currentKey.publicKey);
  validateEcPublicKey(nextKey.publicKey);
  ...
  const pk = importEcPublicKey(currentKey.publicKey, utf8Str(base64urlEncode(derivedFrom)));
  if (!es256Verify(seg.signingInput, seg.signature, pk)) fail(`${ctx}: signature`);
}
...
// Re-export the major-neutral primitives the public index exposes (the v3 forms of
// spec/bap-v3.md §2: jwkEncodePublic/jwkDecodePublic/thumbprint are the EC P-256 spellings from
// ec_jwk.js, not the v1/v2 OKP ones). parseSelector/selectorMatches/Selector and the three domain
// separators are declared here (v3-owned); the rest are the shared modules, imported exactly as
// the v1/v2 façades import them.
...
export type { Bounds, SigningInput, MaximaKey };
